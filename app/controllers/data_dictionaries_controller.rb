class DataDictionariesController < ApplicationController
  ...
  def index
    ...
    # @data_dictionaries = DataDictionary.find_by_sql( "select * from data_dictionaries where id = " + params[:dictid])
    @data_dictionaries = DataDictionary.where( "id > ? AND parent_code = ?", "999", get_parent_id)
    @maxidrecord = @data_dictionaries.order("id desc").limit(1)
    @maxidrecord.each do |data_dictionary|
      set_max_id( data_dictionary.id)
    end
    # "id < ? AND id > ?", @max_id, @min_id  @lastupdate = DataDictionary.find_by_sql( "select * from data_dictionaries where created_at = (select MAX(created_at) from data_dictionaries)" ) #for test
  end
    ...
    respond_to do |format|
      if @data_dictionary.save
        ...
        @lastupdate.each do |lastupd|
          lastupd.update_columns( :id => get_max_id.to_i + 1, :parent_code => get_parent_id, :del_flg => 0 )
        end
